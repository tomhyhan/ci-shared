def call(dockerRepoName, imageName, app) {
    ...
    stages {
        ...
        stage('Scan') {
            steps {
                withCredentials([string(credentialsId: 'Scan', variable: 'TOKEN')]) {
                    sh "docker scan --login --token '$TOKEN'"
                    sh "docker scan --accept-license tomhyhan/${dockerRepoName}:${imageName} --version > ${app}.txt "
